...
export default function Wheel(props: {
  ...
  function slideValues() {
    ...
    for (let i = 0; i < slides; i++) {
      ...
      if (props.type === 'day' && slides === 29) {
        style.transform = `rotateX(${rotate - 36}deg) translateZ(${radius}px)`;
        style.WebkitTransform = `rotateX(${rotate - 36}deg) translateZ(${radius}px)`;
      }
      if (props.type === 'day' && slides === 30) {
        style.transform = `rotateX(${rotate - 18}deg) translateZ(${radius}px)`;
        style.WebkitTransform = `rotateX(${rotate - 18}deg) translateZ(${radius}px)`;
      }
  ...
  return (
    <div className={'wheel keen-slider wheel--perspective-' + perspective} ref={sliderRef}>
      {props.label && <div className='wheel__label'>{props.label}</div>}
      <div
